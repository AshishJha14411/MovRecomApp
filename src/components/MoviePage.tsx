    ...
    backgroundSize: `cover`,
            ...
            <div className=" ml-[-1rem] backdrop-blur-sm">
                {singleMovie && singleMovie.map((item) => {
                    return (
                        <div className="  bg-[#001B41] bg-opacity-50 xl:w-[100%] m-4" key={item.id}>
                            <div className="xl:w-[100%]  z-20 md:w-1/2 p-4">
                                <div className=" h-[100vh]  flex flex-row p-2 rounded-lg" >
                                    <img className=" rounded object-cover mb-6 h-[100%]  w-[50%]" src={`https://image.tmdb.org/t/p/original/${item.poster_path}`} alt="content" />
                                    <div className='p-4 ml-[3rem]'>
                                        ...
                                        <button className='px-[3rem]  text-[#B5F1FA]  py-[1.5rem] font-semibold rounded-full bg-[#0e2a51]' onClick={() => {
                                        if(videoId){
                                            window.open(
